...
# [!] Insert the column names of the target values here
...
# added.
FINAL_UNITS = [3, len(TARGET_COLUMNS)]
...
DEBUG = True
NAMESPACE = 'smiles_multitask_example'
with Skippable(), (e := Experiment(base_path=os.getcwd(), namespace=NAMESPACE, glob=globals())):
        ...
        with open(CSV_PATH, mode='r') as csv_file:
            ...
            for i, row in enumerate(reader):
                ...
                molecule_info = {
                    ...
                    **{key: float(row[key]) for key in TARGET_COLUMNS}
                }
    ...
    for c, (name, data) in enumerate(eye_tracking_dataset.items()):
        ...
        g['graph_labels'] = np.array([float(metadata[key]) for key in TARGET_COLUMNS])
        metadata['index'] = c
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for c, index in enumerate(test_indices):
            ...
            title_string = (f'Element {data["metadata"]["name"]}\n'
                            f'{data["metadata"]["smiles"]}\n'
                            f'True: {out_true[c]} - Pred: {out_pred[c]}')
            if IMPORTANCE_FACTOR != 0:
                title_string += f'\nExplanation Fidelity: {e[f"fidelity/{index}"]:.3f}'
            ...
            for row_index in range(nrows):
                ...
                # Only if we have the explanation co-training enabled, the fidelity values and the
                # dedicated channel names will even exist!
                if IMPORTANCE_FACTOR != 0:
...
with Skippable(), e.analysis:
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        fig, rows = plt.subplots(ncols=n_cols, nrows=1, figsize=(8 * n_cols, 8), squeeze=False)
        fig.suptitle(f'Scatter Plot - TEST SET\n'
                     f'R2: {r2:.3f}')
        for i, column_name in enumerate(TARGET_COLUMNS):
            ax = rows[0][i]

            true_values = [v[i] for v in e['out/true'].values()]
            pred_values = [v[i] for v in e['out/pred'].values()]
            r2_local = r2_score(true_values, pred_values)
            ax.set_title(f'{column_name}\n'
                         f'R2: {r2_local:.2f}')

            ax.set_xlabel('True Value')
            ax.set_ylabel('Predicted Value')
            value_min = min(*true_values, *pred_values)
            value_max = max(*true_values, *pred_values)
