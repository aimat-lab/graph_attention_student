...
class Megan(ks.models.Model):
    ...
    def regression_augmentation(self,
        ...
        if self.regression_bins is not None:
            lo_mask = tf.where(self.regression_bins[0][0] < out_true, 1.0, 0.0) * \
                      tf.where(self.regression_bins[0][1] > out_true, 1.0, 0.0)
            hi_mask = tf.where(self.regression_bins[1][0] < out_true, 1.0, 0.0) * \
                      tf.where(self.regression_bins[1][1] > out_true, 1.0, 0.0)
        else:
